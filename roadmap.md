...
### 31 октября 2025 (сессия 2) - Оптимизация графиков: Автоматическое масштабирование оси RPM ✅

**⚠️ Проблема:** Треть графика была пустой (ось X начиналась с 0, хотя данные с 2600 RPM)

**🎯 Решение:** Реализован алгоритм автоматического определения диапазона RPM

**Алгоритм calculateRpmRange:**
...
**Пример:** Данные 2600-7800 RPM → ось X: 2000-8000 (вместо 0-8000)

**✅ Реализация:**
1. **Создана функция [lib/chartConfig.ts:37-68](frontend/src/lib/chartConfig.ts#L37-L68):**
   - `calculateRpmRange(calculations)` - автоматическое определение диапазона
   - Полностью документирована с JSDoc и примерами

2. **Обновлена функция [lib/chartConfig.ts:134-149](frontend/src/lib/chartConfig.ts#L134-L149):**
   - `createXAxis(name, min?, max?)` - добавлены опциональные параметры

3. **Применено во всех графиках:**
   - ✅ [ChartPreset1.tsx:50,136](frontend/src/components/visualization/ChartPreset1.tsx#L50) - P-Av & Torque
   - ✅ [ChartPreset2.tsx:55,132](frontend/src/components/visualization/ChartPreset2.tsx#L55) - PCylMax
   - ✅ [ChartPreset3.tsx:58,155](frontend/src/components/visualization/ChartPreset3.tsx#L58) - TCylMax & TUbMax
   - ✅ [ChartPreset4.tsx:78,169](frontend/src/components/visualization/ChartPreset4.tsx#L78) - Custom

**✅ Результат:**
- Графики используют пространство эффективно (нет пустых зон)
- Красивые округлённые числа на оси (2000, 8000 вместо 2600, 7834)
- Автоматическая адаптация под любые данные
- Работает для разных двигателей (малые и высокие обороты)

**⏱️ Время:** 35 минут

---

### 31 октября 2025 (сессия 1) - Исправление UI: Отображение названий расчётов ✅

**⚠️ Проблема:** В CalculationSelector показывалось "26 точек данных" вместо названия расчёта

**🔍 Диагностика:**
- В .det файле после `$` идёт название расчёта, которое вводит пользователь (например `$14 UpDate`, `$Base`, `$Turbo 1.5 bar`)
- Парсер ([backend/src/services/fileParser.js:158-175](backend/src/services/fileParser.js#L158-L175)) правильно создавал поля `id` и `name`
- API роут ([backend/src/routes/data.js:231-236](backend/src/routes/data.js#L231-L236)) НЕ передавал поле `name` во frontend
- Frontend отображал только "26 точек данных" (неинформативно для пользователя)

**✅ Решение:**
1. **Исправлен [backend/src/routes/data.js:233](backend/src/routes/data.js#L233):**
   ```javascript
   const calculationsWithMetadata = projectData.calculations.map(calc => ({
     id: calc.id,
     name: calc.name,  // ✅ ДОБАВЛЕНО: Название без $ (например "14 UpDate")
     dataPoints: calc.dataPoints,
     metadata: generateCalculationMetadata(calc.dataPoints)
   }));
   ```

2. **Упрощён [frontend/src/components/visualization/CalculationSelector.tsx:109-117](frontend/src/components/visualization/CalculationSelector.tsx#L109-L117):**
   - Убран текст "26 точек данных" (избыточная информация)
   - Показывается только название расчёта: `{calculation.name}`

**✅ Результат:**
- Пользователь видит **реальное название расчёта** из .det файла
- UI стал информативнее и понятнее
- API теперь корректно передаёт все данные
