...
**Последнее обновление:** 22 октября 2025 (обновление под новую UI спецификацию)
...
- **Этап:** Этап 4 - Frontend Setup + shadcn/ui (ЗАВЕРШЁН ✅)
- **Roadmap версия:** 2.0 (переписан по правилам из "1.8 Планирование и Roadmap.md")
- **Прогресс:** ~58/60+ задач выполнено (96%)
- **Следующее:** Этап 5 - HomePage с карточками проектов (2-3 часа)
- **Изменения:**
  - Roadmap переписан по принципам из документа "1.8 Планирование и Roadmap.md"
  - Задачи разбиты на 15-60 минут (вместо 1-3 часов)
  - Группировка по фичам end-to-end (не по технологиям)
  - Итеративный подход (MVP → расширение)
  - Добавлены секции "🚨 КРИТИЧНО ПЕРЕД НАЧАЛОМ" в каждый этап
  - Ссылки на официальную документацию + напоминания про WebFetch
...
### Этап 6: Frontend - Диалог редактирования метаданных (2-3 часа) 🆕
...
- [ ] Установить необходимые компоненты (15 мин)
  ```bash
...
- [ ] Создать components/metadata/TagInput.tsx (45 мин)
  ```typescript
...
- [ ] Создать components/metadata/MetadataDialog.tsx (1.5 часа)
  ```typescript
  import { useState } from 'react';
...
- [ ] Настроить Toaster в App.tsx (15 мин)
  ```typescript
  ...
  ```
- [ ] Добавить toast функции в api/client.ts (20 мин)
  ```typescript
...
- [ ] Обновить HomePage.tsx с MetadataDialog (30 мин)
  ```typescript
  import { useState } from 'react';
...
- [ ] Тест: создание новых метаданных (20 мин)
  - Открыть проект без метаданных
  - Клик "✏️ Править"
  - Заполнить форму
  - Сохранить → Toast "Метаданные сохранены"
  - Проверить что карточка обновилась
- [ ] Тест: редактирование существующих (15 мин)
  - Открыть проект с метаданными
  - Изменить описание
  - Добавить/удалить теги
  - Сохранить → проверить обновление
- [ ] Тест: валидация (10 мин)
  - Попытка сохранить без описания
  - Проверить alert
  - Проверить лимиты символов (500 для описания)

**Результат Этапа 6:**
✅ MetadataDialog работает
✅ Форма со всеми полями (описание, клиент, теги, статус, заметки)
✅ TagInput компонент (добавление/удаление тегов)
✅ Валидация полей
✅ Toast уведомления (успех/ошибка)
...
### Этап 7: Frontend - Страница визуализации (4-5 часов) 🔄 ОБНОВЛЕНО
