...
**Источник:** EngMod4T simulation software (Delphi 7)
...
### 1. Fixed-width ASCII формат (Delphi origin)

**Формат:** Fixed-width ASCII text с множественными пробелами для выравнивания колонок.

**Происхождение:** Создан Delphi 7 через `WriteLn(F, Format('%12.2f ...', [values]))`.
...
// ❌ НЕПРАВИЛЬНО: Использовать CSV парсер или split по табам
const values = line.split(',');  // НЕТ! Это не CSV
const values = line.split(/\t+/);  // НЕТ! Это не табы
```

### 2. Служебная первая колонка

**Проблема:** Первая колонка содержит символы `№`, `→` и номера строк.

**Решение:** Парсер пропускает первую колонку при обработке всех строк (`slice(1)`).
...
### 5. Вариативное количество точек
